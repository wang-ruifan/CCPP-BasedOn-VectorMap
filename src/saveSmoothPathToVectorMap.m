% filepath: /f:/CCPP-BasedOn-VectorMap/src/saveSmoothPathToVectorMap.m
function saveSmoothPathToVectorMap(smooth_path)
    %% 参数设置
    defaultB = 0;
    defaultL = 0;
    defaultExtra = 0;
    ...
    % 读取原始数据
    pointfile = fullfile(dataDir, 'point.csv');
    nodefile = fullfile(dataDir, 'node.csv');
    lanefile = fullfile(dataDir, 'lane.csv');
    linefile = fullfile(dataDir, 'line.csv');

    tblPoint = readtable(pointfile);
    tblNode = readtable(nodefile);
    tblLane = readtable(lanefile);
    tblLine = readtable(linefile);
