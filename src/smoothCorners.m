% filepath: /CCPP-BasedOn-VectorMap/src/smoothCorners.m
function new_path = smoothCorners(path, params)
    % 对路径中的每个转角进行平滑处理
    % 输入：
    %   path          - 原始路径点集，Nx2 数组
    ...
    smooth_radius = params.radius;
    num_curve_pts = params.numCurvePts;
    angle_threshold = params.angleThreshold;
